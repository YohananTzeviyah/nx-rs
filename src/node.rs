...
pub trait GenericNode<'a> {
    ...
    fn dtype(&self) -> Type;
    /// Gets the string value of this node. This will be `None` if the node is
    /// not a string node.
    fn string(&self) -> Option<&'a str>;
    /// Gets the integer value of this node. This will be `None` if the node
    /// is not an integer node.
    fn integer(&self) -> Option<i64>;
    /// Gets the float value of this node. This will be `None` if the node is
    /// not a float node.
    fn float(&self) -> Option<f64>;
    /// Gets the vector value of this node. This will be `None` if the node is
    /// not a vector node.
    fn vector(&self) -> Option<(i32, i32)>;
    /// Gets the audio value of thise node. This will be `None` if the node is
    /// not an audio node.
    fn audio(&self) -> Option<Audio<'a>>;
    /// Gets the bitmap value of thise node. This will be `None` if the node
    /// is not a bitmap node.
...
impl<'a> Node<'a> {
    /// Creates a Node from the data representing it and the file the data is
    /// from.
    #[inline]
...
impl<'a> GenericNode<'a> for Node<'a> {
    ...
    fn get(&self, name: &str) -> Option<Node<'a>> {
        let mut data = unsafe {
            self.file.get_node(self.data.children) as *const repr::Node
        };
    ...
    fn string(&self) -> Option<&'a str> {
        match self.dtype() {
            Type::String => Some(unsafe {
                self.file.get_str(
                    transmute::<_, repr::String>(self.data.data).index,
                )
            }),
            _ => None,
        }
    ...
    fn integer(&self) -> Option<i64> {
        match self.dtype() {
            Type::Integer => Some(unsafe {
                transmute::<_, repr::Integer>(self.data.data).value
            }),
            _ => None,
        }
    }
    ...
    fn float(&self) -> Option<f64> {
        match self.dtype() {
            Type::Float => Some(unsafe {
                transmute::<_, repr::Float>(self.data.data).value
            }),
            _ => None,
        }
    }
    ...
    fn audio(&self) -> Option<Audio<'a>> {
        match self.dtype() {
            Type::Audio => Some(unsafe {
                ...
                Audio::construct(
                    self.file.get_audio(audio.index, audio.length),
                    audio.index,
                )
            }),
            _ => None,
    ...
    fn bitmap(&self) -> Option<Bitmap<'a>> {
        match self.dtype() {
            Type::Bitmap => Some(unsafe {
                ...
                Bitmap::construct(
                    self.file.get_bitmap(bitmap.index),
                    bitmap.width,
                    bitmap.height,
                )
            }),
